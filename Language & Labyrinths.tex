...
\subsection{Talking about Talking}

\DndDropCapLine{B}{efore we can begin to answer} these questions, a parodox of circularity greets us at the outset: in order to discuss language, we must \textit{use} language. Whether this is an insurmountable challenge or not depends on one's approach.
     
Those following the |Analytic| tradition have typically emphasized the formal characteristics of language. Linguists of this variety tend to prioritize abstraction over synthesis, paying particular attention to a statement's grammar and syntax \parencite{putnam1962analytic}. This is perhaps unsurprising, given that analytic linguistics is commonly motivated by the language of logic and mathematics. A search to apply such rigor to natural language is therefore often sought as a goal.  

    \begin{DndSidebar}[]{Tractatus Logico-Philosophicus} 
    In this seminal 1921 text, Ludwig Wittgenstein attempts to develop a comprehensive model of language--an ideal, nonambiguous language of philosophical inquiry.
    \end{DndSidebar}

How does the inner relate to the outer? But what is form without content?
